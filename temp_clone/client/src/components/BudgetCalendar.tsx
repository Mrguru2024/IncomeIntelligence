      ...
      <Card>
        <CardHeader className="pb-1 px-2 xs:px-4 sm:px-6">
          <div className="flex flex-col justify-between items-start gap-1 xs:gap-2">
            <CardTitle className="text-base xs:text-lg sm:text-2xl">
              Budget Calendar
            </CardTitle>
        ...
        <CardContent className="px-1 xs:px-2 sm:px-6 pt-1 pb-0">
          ...
          <div className="overflow-x-auto -mx-2 px-2 pb-2">
            <div className="min-w-[280px] md:min-w-[500px]">
              {" "}
              {/* Support for ultra-narrow (Z Fold) screens */}
                  ...
                  <div
                    ...
                    title={
                      selectedDayOfWeek === idx
                        ? `Clear ${day.full} filter`
                        : `Show only ${day.full}s`
                    }
                    role="button"
                    tabIndex={0}
                    aria-label={
              ...
              <div className="grid grid-cols-7 gap-0.5 xs:gap-1 mt-0.5 xs:mt-1">
                {days.map((day, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`text-right p-0 xs:p-0.5 sm:p-1 font-medium text-[8px] xs:text-xs ${isToday(day) ? "text-primary font-bold" : ""}`}
                      >
                        {format(day, "d")}
                      </div>
                      ...
                      <div className="text-[8px] xs:text-[10px] sm:text-xs space-y-0.5 sm:space-y-1">
                        ...
                        {dayGoals.map((goal) => (
                          <div
                            key={goal.id}
                            className="bg-blue-50 p-0.5 rounded text-blue-800 text-[7px] xs:text-[9px] truncate"
                          >
                            {goal.name}
                          </div>
                        ))}
        ...
        <CardFooter className="border-t px-1 xs:px-2 sm:px-6 py-2 xs:py-3 sm:py-4">
          <div className="w-full grid grid-cols-2 gap-1 xs:gap-2 sm:gap-4">
            ...
            >
              ...
              <p className="text-sm xs:text-lg sm:text-2xl font-bold text-green-600">
            ...
            >
              ...
              <p className="text-sm xs:text-lg sm:text-2xl font-bold text-blue-600">
                {formatCurrency(calculateGoalsAmount())}
              </p>
            </div>
          </div>
        </CardFooter>
                          ...
                          >
                            <div className="flex justify-between items-center mb-1">
                              ...
                              >
                                {remaining <= 0
                                  ? "Completed!"
                                  : `${formatCurrency(remaining)} needed`}
                              </span>
                            </div>
