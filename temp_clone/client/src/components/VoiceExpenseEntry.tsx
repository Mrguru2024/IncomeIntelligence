...
const VoiceExpenseEntry: React.FC<VoiceExpenseEntryProps> = ({ onSuccess }) => {
  ...
  const submitExpense = () => {
    if (
      ...
      toast({
        ...
        description:
          "Please provide at least a description, amount, and category.",
        variant: "destructive",
      });
    }
  };
  ...
  const processTranscript = () => {
    ...
    if (lowerTranscript.includes("for")) {
      const forMatch = lowerTranscript.match(
        /(?:spent|paid|expense|bought|purchased|for|on) (.*?)(?:for|at|in|with|using|on|$)/i,
      );
      if (forMatch) {
        description = forMatch[1].trim();
      }
    } else if (
      lowerTranscript.includes("spent on") ||
      lowerTranscript.includes("paid for") ||
      lowerTranscript.includes("bought")
    ) {
      const actionMatch = lowerTranscript.match(
        /(?:spent on|paid for|bought) (.*?)(?:for|at|in|with|using|on|$)/i,
      );
      if (actionMatch) {
    ...
    for (const category of expenseCategories) {
      if (
        lowerTranscript.includes(category.id) ||
              ...
              <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                ...
                <dd>
                  {expenseCategories.find(
                    (c) => c.id === parsedExpense.category,
                  )?.name || "Other"}
                </dd>

                <dt className="font-medium text-muted-foreground">Payment:</dt>
                <dd className="capitalize">
                  {parsedExpense.paymentMethod || "Cash"}
                </dd>
              </dl>
            </div>
          ) : (
