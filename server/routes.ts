...
if (stripeSecretKey) {
  ...
  if (process.env.NODE_ENV === "development") {
    console.log(
      "STRIPE_SECRET_KEY not found. Stripe functionality will be disabled in development mode."
    );
  } else {
    console.warn(
...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      if (!validationResult.isValid) {
        ...
        if (validationResult.requiresBilling) {
          errorMessage =
            "The Google Maps API requires billing to be enabled on the Google Cloud Project. Please enable billing at https://console.cloud.google.com/project/_/billing/enable";
    ...
    try {
      ...
      try {
        const addressComplexity =
          ((origin as string).length + (destination as string).length) / 2;
  ...
  app.use(
    express.static(path.join(process.cwd(), "green"), {
      setHeaders: (res, path) => {
  ...
  app.post("/api/create-payment-intent", async (req, res) => {
    try {
      ...
      if (!stripeSecretKey) {
        console.log(
          "STRIPE_SECRET_KEY is not set, returning simulated client secret for development"
        );
  ...
  app.get("/", (req, res) => {
    ...
    if (fs.existsSync(greenHtmlPath)) {
      ...
      console.error(
        "GREEN version HTML file not found at path:",
        greenHtmlPath
      );
      res
        .status(500)
        .send("GREEN version not available. Please check server logs.");
    }
  });
  ...
  app.get("/api/admin/status", requireAuth, async (req, res) => {
    try {
      ...
      googleMapsStatus = process.env.GOOGLE_MAPS_API_KEY
        ? "configured"
        : "not_configured";
      ...
      res.json({
        status: "maintenance",
        message:
          "System is currently in maintenance mode while Firebase dependencies are being removed.",
        services: {
          api: "online",
          database: dbStatus,
          stripe: stripeStatus,
    ...
    try {
      ...
      if (req.user?.id !== userId) {
        return res
          .status(403)
          .json({
            message: "Unauthorized to update this user's onboarding status",
          });
      }
      ...
      res
        .status(500)
        .json({ message: "Failed to update user onboarding status" });
    }
  });
  ...
  app.get("/api/users/:userId/profile", requireAuth, async (req, res) => {
    try {
      ...
      if (req.user?.id !== userId) {
        return res
          .status(403)
          .json({ message: "Unauthorized to view this user profile" });
      }
  ...
  app.post("/api/users/:userId/profile", requireAuth, async (req, res) => {
    try {
      ...
      if (req.user?.id !== userId) {
        return res
          .status(403)
          .json({ message: "Unauthorized to update this user profile" });
      }
  ...
  app.post("/api/plaid/create-link-token", async (req, res) => {
    try {
      ...
      if (!process.env.PLAID_CLIENT_ID || !process.env.PLAID_SECRET) {
        return res.status(500).json({
          error:
            "Plaid configuration is missing. Please check your environment variables.",
        });
      }
  ...
  app.post("/api/plaid/exchange-token", async (req, res) => {
    try {
      ...
      const connectionId = await plaidService.exchangePublicToken(
        userId,
        publicToken,
        metadata
      );
  ...
  app.post("/api/bank-connections/:id/import-income", async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: "Transactions imported as income successfully",
      });
    } catch (error) {
      console.error("Error importing transactions as income:", error);
      res
        .status(500)
        .json({ message: "Failed to import transactions as income" });
    }
  });
  ...
  app.post("/api/guardrails/spending-limits", async (req, res) => {
    try {
      ...
      if (period !== "weekly" && period !== "monthly") {
        return res
          .status(400)
          .json({ message: "Period must be either 'weekly' or 'monthly'" });
      }
      ...
      if (success) {
        ...
        return res
          .status(500)
          .json({ message: "Failed to add spending limit" });
      }
    } catch (error) {
  ...
  app.put("/api/guardrails/spending-limits/:limitId", async (req, res) => {
    try {
      ...
      const success = await storage.updateSpendingLimit(
        userIdNum,
        limitId,
        updates
      );
      ...
      if (success) {
        ...
        const updatedLimit = spendingLimits.find(
  ...
  app.delete("/api/guardrails/spending-limits/:limitId", async (req, res) => {
    try {
      ...
      if (success) {
        return res
          .status(200)
          .json({ message: "Spending limit deleted successfully" });
      } else {
        return res.status(404).json({ message: "Spending limit not found" });
      }
    } catch (error) {
    ...
    try {
      ...
      try {
        if (validatedData.userId && validatedData.category) {
          ...
          if (
            userData &&
            userData.spendingLimits &&
            Array.isArray(userData.spendingLimits)
          ) {
        ...
        console.error(
          "Error processing guardrails for expense:",
          guardrailsError
        );
      }
  ...
  app.get("/api/users/:userId/budgets", requireAuth, async (req, res) => {
    try {
      ...
      if (req.user?.id !== userId) {
        return res
          .status(403)
          .json({ message: "Unauthorized to view these budgets" });
      }
  ...
  app.get(
    "/api/users/:userId/budgets/:year/:month",
    requireAuth,
    async (req, res) => {
      try {
        const userId = parseInt(req.params.userId);
        ...
        const budgets = await storage.getBudgetsByYearMonth(
          userId,
          year,
          month
        );
  ...
  app.patch("/api/gigs/:id", requireAuth, async (req, res) => {
    try {
      ...
      if (req.user?.id !== existingGig.createdBy) {
        return res
          .status(403)
          .json({ message: "Unauthorized to update this gig" });
      }
  ...
  // Join affiliate program
  app.post(
    "/api/affiliate-programs/:id/join",
    requireAuth,
    async (req, res) => {
      try {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to record affiliate conversion" });
      }
    }
  );
  ...
  app.patch(
    "/api/user-challenges/:id/progress",
    requireAuth,
    async (req, res) => {
      try {
        const id = parseInt(req.params.id);
        ...
        res
          .status(500)
          .json({ message: "Failed to update challenge progress" });
      }
    }
  );
    ...
    async (req, res) => {
      try {
        ...
        if (!stripe) {
          return res
            .status(500)
            .json({ message: "Payment processing is not available" });
        }
  ...
  app.patch("/api/used-gear/:id", requireAuth, async (req, res) => {
    try {
      ...
      const validatedData = insertUsedGearListingSchema
        .partial()
        .parse(req.body);
    ...
    async (req, res) => {
      try {
        if (!stripe) {
          return res
            .status(503)
            .json({ message: "Stripe payment processing is not available" });
        }
        ...
        if (invoice.paid) {
          return res
            .status(400)
            .json({ message: "This invoice has already been paid" });
        }
  ...
  app.get(
    "/api/users/:userId/invoice-summary",
    requireAuth,
    async (req, res) => {
      try {
        const userId = req.params.userId;
        if (req.user?.id !== userId) {
          return res
            .status(403)
            .json({ message: "Unauthorized to access this data" });
        }
  ...
  app.get(
    "/api/users/:userId/grant-applications",
    requireAuth,
    async (req, res) => {
      try {
        const userId = req.params.userId;
        ...
        res
          .status(500)
          .json({ message: "Failed to get user grant applications" });
      }
    }
  );
  ...
  app.get("/api/balances/user/:userId/month/:year/:month", async (req, res) => {
    try {
      ...
      if (!balance) {
        ...
        const calculatedBalance = await storage.calculateCurrentBalance(
          userId,
          year,
          month
        );
  ...
  app.post("/api/balances", async (req, res) => {
    try {
      ...
      if (existingBalance) {
        ...
        balance = await storage.updateBalance(
          existingBalance.id,
          validatedData
        );
      } else {
  ...
  app.post("/api/ai/financial-advice", requireAuth, async (req, res) => {
    try {
      ...
      const balanceData = await storage.getBalance(
        userId,
        currentYear,
        currentMonth
      );
      ...
      if (advice.error) {
        const statusCode =
          advice.errorType === "quota_exceeded" ||
  ...
  app.post("/api/notifications/check-reminders", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to process reminder notifications" });
    }
  });
  ...
  app.patch("/api/user/widget-settings", async (req, res) => {
    try {
      ...
      const validatedData = insertWidgetSettingsSchema
        .partial()
        .parse(req.body);
    ...
    try {
      const schema = z.object({
        ...
        blogContent: z
          ...
          .min(
            20,
            "Blog content must be at least 20 characters for proper image generation"
          ),
    ...
    async (req, res) => {
      try {
        ...
        if (req.user?.id !== userId) {
          return res
            .status(403)
            .json({ message: "Unauthorized to submit quiz for this user" });
        }
        ...
        const result = await spendingPersonalityService.submitQuiz(
          userId,
          validatedAnswers
        );
  ...
  app.get(
    "/api/spending-personality/results/:userId",
    requireAuth,
    checkUserMatch,
    async (req, res) => {
      try {
  ...
  app.get(
    "/api/spending-personality/latest-result/:userId",
    requireAuth,
    checkUserMatch,
    async (req, res) => {
      try {
        ...
        const result =
          await spendingPersonalityService.getLatestUserResult(userId);
        if (!result) {
          return res
            .status(404)
  ...
  app.post(
    "/api/spending-personality/questions",
    requireAuth,
    requireAdmin,
    async (req, res) => {
      try {
        const question = await spendingPersonalityService.createQuestion(
          req.body
        );
  ...
  app.get("/api/subscription/:userId", requireAuth, async (req, res) => {
    try {
      ...
      if (req.user?.id !== userId && req.user?.role !== "admin") {
        return res
          .status(403)
          .json({ message: "Unauthorized to view this subscription" });
      }
      ...
      res
        .status(500)
        .json({ message: "Failed to get subscription information" });
    }
  });
  ...
  if (stripe) {
    ...
    app.post("/api/create-subscription", requireAuth, async (req, res) => {
      try {
        ...
        if (!user.email) {
          return res
            .status(400)
            .json({ message: "User email is required for subscription" });
        }
        ...
        if (!priceId) {
          return res
            .status(400)
            .json({ message: "No subscription price ID provided" });
        }
        ...
        return res
          .status(500)
          .json({ message: "Subscription creation failed" });
      }
    });
      ...
      async (req, res) => {
        ...
        if (!signature || !process.env.STRIPE_WEBHOOK_SECRET) {
          return res
            ...
            .json({
              message:
                "Webhook signature missing or webhook secret not configured",
            });
        }
    ...
    app.get("/api/verify-subscription", requireAuth, async (req, res) => {
      try {
          ...
          try {
            ...
            if (isActive) {
              ...
              const subscriptionInfo = {
                ...
                plan:
                  subscription.items.data[0]?.price.nickname || "Stackr Pro",
        ...
        return res
          .status(500)
          .json({ message: "Failed to verify subscription" });
      }
    });
    ...
    app.post("/api/create-payment-intent", requireAuth, async (req, res) => {
      try {
        ...
        if (!user.email) {
          return res
            .status(400)
            .json({ message: "User email is required for payment" });
        }
  ...
  // Get user's products
  app.get(
    "/api/users/:userId/digital-products",
    requireAuth,
    async (req, res) => {
      try {
        const userId = parseInt(req.params.userId);
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch user digital products" });
      }
    }
  );
  ...
  app.get("/api/investment-strategies", requireAuth, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch investment strategies" });
    }
  });

  // Get recommended investment strategies - requires Pro subscription
  app.get(
    "/api/investment-strategies/recommended",
    requireAuth,
    requireProSubscription,
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch recommended strategies" });
      }
    }
  );
  ...
  // Get user's listings
  app.get(
    "/api/users/:userId/used-gear-listings",
    requireAuth,
    async (req, res) => {
      try {
        const userId = parseInt(req.params.userId);
  ...
  app.post(
    "/api/creative-grants/:grantId/applications",
    requireAuth,
    requireProSubscription,
    async (req, res) => {
      try {
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        ...
        const validatedData = insertDiscountCodeSchema
          .partial()
          .parse(req.body);
  ...
  app.post("/api/apply-discount", requireAuth, async (req, res) => {
    try {
      ...
      if (!code || !planId) {
        return res
          .status(400)
          .json({ message: "Discount code and plan ID are required" });
      }
  ...
  app.get("/api/users/:userId/subscription", requireAuth, async (req, res) => {
    try {
      ...
      if (req.user?.id !== userId) {
        return res
          .status(403)
          .json({ message: "Unauthorized to view this user's subscription" });
      }
  ...
  app.get("/api/professional-services/:id", async (req, res) => {
    try {
      ...
      if (!service) {
        return res
          .status(404)
          .json({ message: "Professional service not found" });
      }
  ...
  app.patch("/api/professional-services/:id", requireAuth, async (req, res) => {
    try {
      ...
      if (service.userId !== req.user?.id) {
        return res
          .status(403)
          .json({ message: "Unauthorized - You do not own this service" });
      }

      const validatedData = insertProfessionalServiceSchema
        .partial()
        .parse(req.body);

      const updatedService = await storage.updateProfessionalService(
        id,
        validatedData
      );
      if (!updatedService) {
      ...
      res
        .status(500)
        .json({ message: "Failed to update professional service" });
    }
  });
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to delete professional service" });
      }
    }
  );
  ...
  app.get("/api/users/:userId/professional-services", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch user professional services" });
    }
  });
  ...
  app.get("/api/professional-services/category/:category", async (req, res) => {
    try {
      ...
      const services =
        await storage.getProfessionalServicesByCategory(category);
      res.json(services);
    } catch (error) {
      console.error("Error fetching professional services by category:", error);
      res
        .status(500)
        .json({ message: "Failed to fetch professional services by category" });
    }
  });

  // Toggle service active status (requires authentication and ownership)
  app.patch(
    "/api/professional-services/:id/toggle-active",
    requireAuth,
    async (req, res) => {
      try {
        const id = parseInt(req.params.id);
        ...
        const updatedService = await storage.toggleProfessionalServiceActive(
          id,
          isActive
        );
        if (!updatedService) {
        ...
        console.error(
          "Error toggling professional service active status:",
          error
        );
        res
          .status(500)
          .json({
            message: "Failed to toggle professional service active status",
          });
      }
    }
  );
  ...
  app.get("/clean", (req, res) => {
    try {
      ...
      res.setHeader(
        ...
        "default-src 'self'; script-src 'nonce-stackr-safe' 'self'; connect-src 'self'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data:;"
      );
...
<body>
  ...
  app.get("/standalone", (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Security-Policy",
        "default-src 'self'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; connect-src 'none';"
      );
      res.setHeader("X-Content-Type-Options", "nosniff");
  ...
  app.get("/firebase-free", (req, res) => {
    try {
      const standaloneHtmlPath = path.resolve(
        process.cwd(),
        "client",
        "standalone.html"
      );
  ...
  app.get("/api/green/affiliates", (req, res) => {
    try {
      const affiliatesDataPath = path.join(
        process.cwd(),
        "green",
        "affiliates.js"
      );
  ...
  app.get("/api/bank-accounts/user/:userId", requireAuth, async (req, res) => {
    try {
      ...
      if (req.user?.id !== userId) {
        return res
          .status(403)
          .json({ message: "Unauthorized to view these bank accounts" });
      }
  ...
  app.get("/api/invoices/:id/pdf", requireAuth, async (req, res) => {
    try {
      ...
      if (req.user?.id !== invoice.userId) {
        return res
          .status(403)
          .json({ message: "Unauthorized to access this invoice" });
      }
    ...
    try {
      ...
      if (!["email", "sms", "both"].includes(deliveryMethod)) {
        return res
          ...
          .json({
            message:
              "Invalid delivery method. Must be 'email', 'sms', or 'both'",
          });
      }
      ...
      if (req.user?.id !== invoice.userId) {
        return res
          .status(403)
          .json({ message: "Unauthorized to send this invoice" });
      }
      ...
      if (
        ...
        return res
          .status(400)
          .json({ message: "Invoice has no client email address" });
      }
      ...
      if (
        ...
        return res
          .status(400)
          .json({ message: "Invoice has no client phone number" });
      }

      // Import the invoice service dynamically to avoid circular dependencies
      const { sendInvoice, sendSms } = await import("./email-service");
      const { sendInvoiceEmail, sendInvoiceSms } = await import(
      ...
      if (deliveryMethod === "both") {
        if (sendResult.email && sendResult.sms) {
          ...
          res.json({
            message:
              "Invoice sent successfully via email only. SMS delivery failed.",
            emailSent: true,
            smsSent: false,
          });
        } else {
          res.json({
            message:
              "Invoice sent successfully via SMS only. Email delivery failed.",
            emailSent: false,
            smsSent: true,
          });
        }
        ...
        switch (event.type) {
          ...
          case "payment_intent.payment_failed":
            ...
            if (
              ...
              failedPaymentIntent.metadata.invoiceId
            ) {
  ...
  app.get("/api/google-maps-direct", async (req, res) => {
    try {
      ...
      console.log(
        "Successfully served direct Google Maps script URL with callback=initGooglePlacesAPI"
      );
    } catch (error) {
  ...
  app.get("/api/address-suggestions", async (req, res) => {
    try {
      ...
      function generateFallbackSuggestions(query: string) {
        ...
        for (let i = 0; i < Math.min(5, cities.length); i++) {
          ...
          if (query.match(/^\d+/)) {
            ...
            query.match(/\b(?:street|st|avenue|ave|road|rd|boulevard|blvd)\b/i)
          ) {
      ...
      if (!apiKey) {
        console.warn(
          "Google Maps API key not configured, using fallback suggestions"
        );
      ...
      res.json({
        predictions: [
          {
            description: `${req.query.query}, CA, USA`,
            place_id: "ca-" + Date.now(),
          },
          {
            description: `${req.query.query}, NY, USA`,
            place_id: "ny-" + Date.now(),
          },
          {
            description: `${req.query.query}, TX, USA`,
            place_id: "tx-" + Date.now(),
          },
        ],
      });
    }
...
async function validateGoogleMapsApiKey(): Promise<{
  isValid: boolean;
  errorMessage?: string;
}> {
  try {
