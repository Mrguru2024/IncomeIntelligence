...
export const signInWithSocial = async (provider: SocialAuthProvider): Promise<UserCredential> => {
  try {
    let authProvider;
...
export const signInWithSocialRedirect = async (provider: SocialAuthProvider): Promise<void> => {
  try {
    let authProvider;
...
const sendAuthToBackend = async (credentials: UserCredential): Promise<void> => {
  try {
    // Get the ID token
