            ...
            <div key={income.id} className="p-4">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                <span className="bg-blue-100 text-blue-800 text-xs px-1.5 py-0.5 rounded">
                  {formatCurrency(amount * 0.4)}
                </span>
                <span className="bg-purple-100 text-purple-800 text-xs px-1.5 py-0.5 rounded">
                  {formatCurrency(amount * 0.3)}
                </span>
                <span className="bg-green-100 text-green-800 text-xs px-1.5 py-0.5 rounded">
                  {formatCurrency(amount * 0.3)}
                </span>
                <span
                  className={`text-xs px-1.5 py-0.5 rounded ml-auto ${income.source === "Manual" ? "bg-gray-100 text-gray-800" : "bg-blue-100 text-blue-800"}`}
                >
                  {income.source}
                </span>
              </div>
    ...
    <Card className="border border-gray-100 mb-6 sm:mb-8">
      <CardContent className="p-4 sm:p-6">
        <div className="flex justify-between items-center mb-3 sm:mb-4">
          ...
          <Link
            href="/income-history"
            className="text-primary hover:text-blue-700 text-xs sm:text-sm font-medium"
          >
            View All
          </Link>
        </div>
                ...
                recentIncomes.map((income) => {
                  ...
                  return (
                    <tr key={income.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900 font-medium">
                        {formatCurrency(amount)}
                      </td>
                      <td className="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500">
                        <div className="flex items-center space-x-1 sm:space-x-2">
                          <span className="bg-blue-100 text-blue-800 text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded">
                            {formatCurrency(needsAmount)}
                          </span>
                          <span className="bg-purple-100 text-purple-800 text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded">
                            {formatCurrency(investmentsAmount)}
                          </span>
                          <span className="bg-green-100 text-green-800 text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded">
                      ...
                      <td className="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500">
                        <span
                          className={`text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded ${income.source === "Manual" ? "bg-gray-100 text-gray-800" : "bg-blue-100 text-blue-800"}`}
                        >
                          {income.source}
                        </span>
                      </td>
