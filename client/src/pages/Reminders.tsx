...
// Icon imports
import { 
  Bell, CalendarDays, Clock, AlertCircle, Check, X, Calendar as CalendarIcon, 
  Edit2, Trash2, ArrowUpRight, ChevronDown, Filter, PlusCircle
} from 'lucide-react';
...
export default function Reminders() {
  ...
  const updateReminderMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: ReminderFormValues }) => {
  ...
  const getReminderType = (typeId: string) => {
    return reminderTypes.find(type => type.id === typeId) || { 
      id: 'custom', 
      name: 'Custom', 
      icon: 'bell', 
      color: 'amber' 
  ...
  const getFrequencyName = (frequencyId: string) => {
    return reminderFrequencies.find(freq => freq.id === frequencyId)?.name || 'Custom';
  };
    ...
    <main className="p-4 md:p-6 lg:p-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Manage your financial tasks and reminders</p>
        </div>
            ...
            return (
              <div key={date} className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  {isPast && !isToday && <Badge variant="destructive">Overdue</Badge>}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {groupedReminders[date].map((reminder: ReminderData) => {
                    ...
                    return (
                      <Card key={reminder.id} className={`border ${!reminder.isActive ? 'opacity-60' : ''}`}>
                        <CardHeader className="pb-2">
                          <div className="flex justify-between items-start">
                            <div className="flex items-center gap-2">
                              <Badge className={`bg-${reminderType.color}-100 text-${reminderType.color}-800 hover:bg-${reminderType.color}-200`}>{reminderType.name}</Badge>
                              {!reminder.isActive && (
                                <Badge variant="outline" className="text-amber-800">Paused</Badge>
                              )}
                            </div>
                          ...
                          <CardDescription className="line-clamp-2">{reminder.message}</CardDescription>
                        </CardHeader>
                        <CardContent className="pb-2">
                          <div className="flex flex-col space-y-1 text-sm text-muted-foreground">
                            <div className="flex items-center gap-2">
                              ...
                              <span>
                                {format(new Date(reminder.nextRemindAt), 'h:mm a')}
                              </span>
                            </div>
                            <div className="flex items-center gap-2">
                        ...
                        <CardFooter>
                          {reminder.lastSentAt && (
                            <div className="text-xs text-muted-foreground flex items-center gap-1">
                              ...
                              <span>Last sent: {format(new Date(reminder.lastSentAt), 'MMM d, yyyy')}</span>
                            </div>
                          )}
                        </CardFooter>
                      </Card>
