...
jest.mock("@/components/ui/card", () => ({
  ...
  CardHeader: ({
    ...
    children: React.ReactNode;
    className?: string;
  }) => <div className={`mock-card-header ${className || ""}`}>{children}</div>,
  CardTitle: ({
    children,
    className,
  }: {
    children: React.ReactNode;
    className?: string;
  }) => <div className={`mock-card-title ${className || ""}`}>{children}</div>,
  CardContent: ({
    children,
    className,
  }: {
...
jest.mock("@/components/ui/progress", () => ({
  Progress: ({ value, className }: { value: number; className?: string }) => (
    <div className={`mock-progress ${className || ""}`} data-value={value}>
      Progress
    </div>
  ),
}));
