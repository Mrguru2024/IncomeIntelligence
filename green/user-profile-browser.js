...
// Define a self-executing function to avoid polluting global scope
(function() {
  ...
  function loadCurrentUserProfile() {
    ...
    try {
      const userInfo = JSON.parse(sessionStorage.getItem('stackr_user_info') || '{}');
      if (userInfo && userInfo.userId) {
        return getUserProfile(userInfo.userId);
      }
      
      // If no user in session, try the default user
      const defaultUser = getUserProfile('default');
      if (defaultUser) return defaultUser;
      
      // If no default user, check if we can create one based on username in session
      if (userInfo && userInfo.username) {
        const newUserId = userInfo.userId || `user-${userInfo.username}`;
        const newProfile = initializeUserProfile(newUserId);
        newProfile.name = userInfo.username;
        saveUserProfile(newUserId, newProfile);
        return newProfile;
      }
      
      console.warn('No user profile found and unable to initialize one');
      return null;
    } catch (error) {
  ...
  window.UserProfileModule = UserProfile; // For compatibility
})();

console.log('UserProfile module loaded for browser');