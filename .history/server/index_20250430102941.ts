...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  if (
    req.path.includes("/src/main.tsx") ||
    ...
    if (path.startsWith("/api")) {
      ...
      if (capturedJsonResponse) {
        ...
        if (sanitizedResponse.accessToken)
          sanitizedResponse.accessToken = "[REDACTED]";
        if (sanitizedResponse.authToken)
          sanitizedResponse.authToken = "[REDACTED]";
...
app.get("/blog/editor/static", (req, res, next) => {
  ...
  if (
    req.query.auth === "true" ||
    req.headers.cookie?.includes("authenticated")
  ) {
    const blogEditorPath = path.join(
      process.cwd(),
      "public",
      "blog-editor.html"
    );
