...
export function renderReferralProgram(tier) {
  ...
  requirementNote.textContent =
    "Note: Credit is only earned when your referred friend subscribes to a paid tier (Pro or Lifetime).";
  ...
  // Bonus thresholds
  if (
    tier.referralProgram.bonusThresholds &&
    tier.referralProgram.bonusThresholds.length > 0
  ) {
  ...
  if (tier.referralProgram.lifetimeReferralBonus) {
    maxInfo.innerHTML +=
      ' <span style="color: #10b981; font-weight: bold;">(Unlimited for Lifetime members!)</span>';
  }
  ...
  messageBox.textContent =
    "I've been using Stackr to manage my finances and thought you might like it too! Use my referral link for 1 month of Pro access free.";
...
export async function upgradeMembership(tierId, billingInterval = "monthly") {
    ...
    ${
      billingInterval === "monthly"
        ...
        : billingInterval === "yearly"
          ? `$${tier.yearlyPrice}/year (save ${Math.round((1 - tier.yearlyPrice / (tier.monthlyPrice * 12)) * 100)}%)`
  ...
  if (confirmed) {
    ...
    createToast(
      `Redirecting to ${tier.name} ${billingInterval} checkout...`,
      "info"
    );
...
function renderTierCard(tier, billingInterval, currentTier) {
  ...
  card.style.boxShadow = isCurrentTier
    ? `0 4px 14px 0 rgba(${hexToRgb(tier.color)}, 0.25)`
  ...
  if (!isCurrentTier) {
    ...
    button.addEventListener("click", () => {
      if (tier.id === "free") {
        ...
        const confirmed = confirm(
          'Are you sure you want to downgrade to the Free tier? You"ll lose access to premium features.'
        );
        if (confirmed) {
 ...
 */
export function renderMembershipUpgradePage(
  containerId = "app",
  ...
  closeButton.addEventListener("click", (e) => {
    ...
    try {
      ...
      if (
        document
          .getElementById("app")
          .innerHTML.includes("Stackr Finance Membership")
      ) {
  ...
  subtitle.textContent =
    'Choose the plan that"s right for you and take your financial journey to the next level.';
  ...
  if (
    Object.values(MEMBERSHIP_TIERS).some(
      (tier) => tier.monthlyPrice && tier.yearlyPrice
    )
  ) {
    const monthlyLabel = document.createElement("span");
    monthlyLabel.textContent = "Monthly";
    monthlyLabel.style.color =
      billingInterval === "monthly" ? "#111827" : "#6b7280";
    monthlyLabel.style.fontWeight =
      billingInterval === "monthly" ? "600" : "400";
    ...
    toggleSwitch.setAttribute(
      "aria-checked",
      billingInterval === "yearly" ? "true" : "false"
    );
    ...
    toggleBall.style.transform =
      billingInterval === "yearly" ? "translateX(24px)" : "translateX(0)";
    ...
    toggleSwitch.addEventListener("click", () => {
      ...
      toggleBall.style.transform = isYearly
        ? "translateX(0)"
        : "translateX(24px)";
    ...
    yearlyLabel.style.color =
      billingInterval === "yearly" ? "#111827" : "#6b7280";
 ...
 */
export function renderQuickUpgradeButton(
  container,
  tierId = "pro",
  variant = "button",
  size = "medium"
